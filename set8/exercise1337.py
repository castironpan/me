...
def string_list_please() -> list:
    ...
    return ["I","Love","Orangutans","And","Love"]


def dictionary_please() -> dict:
    """Returns a dictionary, anything you like."""
    return {"Orang": "Utan", "Cina": "Orang", "Melayu": "Bumiputera"}
...
def take_five(some_number) -> int:
    ...
    subtracted_five = some_number - 5

    return subtracted_five
...
def n_counter(search_for_this, input_list=[1, 4, 1, 5, 1, 1]) -> int:
    ...
    for whatever_this_is in input_list:
        if whatever_this_is == search_for_this:
            count += 1
...
def best_letter_for_pets() -> str:
    ...
    length_of_longest_list = len(pet_filter('a'))
    most_popular_letter = 'a'

    for letter in the_alphabet:
        if (len(pet_filter(letter)) > length_of_longest_list):
            length_of_longest_list = len(pet_filter(letter))
